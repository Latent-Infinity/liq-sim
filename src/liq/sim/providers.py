...
def fee_model_from_config(
    cfg: ProviderConfig,
) -> TieredMakerTakerFee | ZeroCommissionFee | PerShareFee:
    ...
    if cfg.fee_model == "PerShare":
        ...
        min_per_order = (
            Decimal(str(cfg.fee_params.get("min_per_order")))
            if "min_per_order" in cfg.fee_params
            else None
        )
...
def slippage_model_from_config(
    cfg: ProviderConfig,
) -> VolumeWeightedSlippage | PFOFSlippage | SpreadBasedSlippage:
